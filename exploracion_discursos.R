...
# Tercer intento. muerte tambien ####
  ...
  # siguiendo los pasos de:
  # https://cmdlinetips.com/2019/04/introduction-to-pca-with-r-using-prcomp/
  pca_resv3 <- prcomp(candidatos_dtmv3, scale=TRUE)
  summary(pca_resv3)
  pca_resv3$x[1:5,1:3]
...
grafo_menciones_dirigido <- plot.igraph(grafo_menciones_dirigido_df,
                              ...
                              vertex.label.dist=2,
                              vertex.label.cex=degree(grafo_menciones_dirigido_df,mode = "in")*2,
...
candidatos_hashtags <- candidatos_campaña %>% 
  tokenizarTextoTuits() %>% 
